...
document.getElementById("confirm-button").addEventListener("click", confirmMeeting);
document.getElementById("back-button").addEventListener("click", goBackToCalendar);
document.getElementById("cancel-button").addEventListener("click", cancelMeeting);
document.getElementById("reschedule-button").addEventListener("click", rescheduleMeeting);
...
function confirmMeeting() {
  ...
  const participantsText = participants.length > 0 ? participants.map(p => `${p.name} (${p.email})`).join(', ') : "Nenhum";
